...
(function (global, factory) {
  ...
  */
  ...
  /**
   * --------------------------------------------------------------------------
   * Bootstrap util/ProgettoIUM-factory.js
   * Licensed under MIT (https://github.com/twbs/bootstrap/blob/main/LICENSE)
   * --------------------------------------------------------------------------
   */
  ...
  const NAME$5 = 'ProgettoIUMFactory';
  const Default$4 = {
  ...
  class ProgettoIUMFactory extends Config {
    constructor(config) {
      super();
      this._config = this._getConfig(config);
    }
    ...
    toHtml() {
      const ProgettoIUMWrapper = document.createElement('div');
      ProgettoIUMWrapper.innerHTML = this._maybeSanitize(this._config.ProgettoIUM);
      for (const [selector, text] of Object.entries(this._config.content)) {
        this._setContent(ProgettoIUMWrapper, text, selector);
      }
      const ProgettoIUM = ProgettoIUMWrapper.children[0];
      const extraClass = this._resolvePossibleFunction(this._config.extraClass);
      if (extraClass) {
        ProgettoIUM.classList.add(...extraClass.split(' '));
      }
      return ProgettoIUM;
    }
    ...
    _setContent(ProgettoIUM, content, selector) {
      const ProgettoIUMElement = SelectorEngine.findOne(selector, ProgettoIUM);
      if (!ProgettoIUMElement) {
        return;
      }
      content = this._resolvePossibleFunction(content);
      if (!content) {
        ProgettoIUMElement.remove();
        return;
      }
      if (isElement$1(content)) {
        this._putElementInProgettoIUM(getElement(content), ProgettoIUMElement);
        return;
      }
      if (this._config.html) {
        ProgettoIUMElement.innerHTML = this._maybeSanitize(content);
        return;
      }
      ProgettoIUMElement.textContent = content;
    }
    _maybeSanitize(arg) {
    ...
    _putElementInProgettoIUM(element, ProgettoIUMElement) {
      if (this._config.html) {
        ProgettoIUMElement.innerHTML = '';
        ProgettoIUMElement.append(element);
        return;
      }
      ProgettoIUMElement.textContent = element.textContent;
    }
  }
  ...
  const Default$3 = {
    ...
    sanitizeFn: null,
    selector: false,
    ProgettoIUM: '<div class="tooltip" role="tooltip">' + '<div class="tooltip-arrow"></div>' + '<div class="tooltip-inner"></div>' + '</div>',
    title: '',
    trigger: 'hover focus'
  };
  const DefaultType$3 = {
  ...
  class Tooltip extends BaseComponent {
    ...
    _getTipElement() {
      if (!this.tip) {
        this.tip = this._createTipElement(this._newContent || this._getContentForProgettoIUM());
      }
      return this.tip;
    }
    _createTipElement(content) {
      const tip = this._getProgettoIUMFactory(content).toHtml();
    ...
    _getProgettoIUMFactory(content) {
      if (this._ProgettoIUMFactory) {
        this._ProgettoIUMFactory.changeContent(content);
      } else {
        this._ProgettoIUMFactory = new ProgettoIUMFactory({
          ...this._config,
      ...
      return this._ProgettoIUMFactory;
    }
    _getContentForProgettoIUM() {
      return {
  ...
  const Default$2 = {
    ...
    offset: [0, 8],
    placement: 'right',
    ProgettoIUM: '<div class="popover" role="tooltip">' + '<div class="popover-arrow"></div>' + '<h3 class="popover-header"></h3>' + '<div class="popover-body"></div>' + '</div>',
    trigger: 'click'
  };
  const DefaultType$2 = {
    ...Tooltip.DefaultType,
